...
function Modal({ isOpen, onClose, event, isLoggedIn, onLogin }) {
  ...
  const handleBookNow = () => {
    if (data) {
      dispatch(attendEventAction({ eventID: event.eventID, userID: data.id, amount: event.amount || null }));
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[99]">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full relative h-[90%] overflow-auto">
        ...
        <div className="mt-4">
          ...
          <button className="mt-4 bg-blue-500 text-white py-2 px-4 rounded" onClick={handleBookNow}>
            Book Now
          </button>
        </div>
      </div>
